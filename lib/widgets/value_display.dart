...
class ValueDisplay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final formatted = formatBytes(bytes, format);
  ...
  static String formatBytes(Uint8List bytes, DisplayFormat format) {
    switch (format) {
  ...
  static String _toHexDump(Uint8List bytes) {
    ...
    for (var i = 0; i < bytes.length; i += 16) {
      ...
      buffer.write('|');
      if (i + 16 < bytes.length) buffer.writeln();
    }
